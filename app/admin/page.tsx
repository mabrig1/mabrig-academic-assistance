...
export default async function AdminPage() {
  ...
  return (
    <main className="section container">
      ...
      <section className="section"><div className="card"><h2>Recent Orders</h2><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><th style={{textAlign:"left",padding:10}}>Order</th><th style={{textAlign:"left",padding:10}}>Student</th><th style={{textAlign:"left",padding:10}}>Service</th><th style={{textAlign:"left",padding:10}}>Status</th><th style={{textAlign:"left",padding:10}}>Created</th></tr></thead><tbody>{enriched.map(order => <tr key={String(order._id)}><td style={{padding:10,borderTop:"1px solid #dce7e2"}}>{order.orderNumber}</td><td style={{padding:10,borderTop:"1px solid #dce7e2"}}>{order.userName}</td><td style={{padding:10,borderTop:"1px solid #dce7e2"}}>{order.serviceName}</td><td style={{padding:10,borderTop:"1px solid #dce7e2"}}>{order.status}</td><td style={{padding:10,borderTop:"1px solid #dce7e2"}}>{new Date(order.createdAt).toLocaleString()}</td></tr>)}</tbody></table></div></div></section>
    </main>
  );
}
